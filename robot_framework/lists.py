...
def send_driftliste(orchestrator_connection: OrchestratorConnection, process_name):
    ...
    while n < len(globals.driftliste):
        body = body + str(n+1)+": Reference: "+str(globals.driftliste[n]["reference"])+" - Status: "+str(globals.driftliste[n]["status"])+" - Message: "+str(globals.driftliste[n]["message"])[0:50]+" - Dannet af: "+str(globals.driftliste[n]["created_by"])+"<br>"
        n += 1
