...
def send_driftliste(orchestrator_connection: OrchestratorConnection, process_name):
    ...
    while n < len(globals.driftliste):
        body = body + globals.driftliste[n]["status"]+"\t"+globals.driftliste[n]["message"]+"\t"+str(globals.driftliste[n]["start_date"])+"\t"+globals.driftliste[n]["created_by"]+"\t"+globals.driftliste[n]["data"]+"<br>"
        n += 1
